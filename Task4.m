...
for i = 1:length(EbN0_dB)
    ...
    while (error < 500) && (noOfL < 1e4)
        ...
        symbols_I = PAM4_mod(bits_I);
        symbols_Q = PAM4_mod(bits_Q);
        ...
        [rx_I, rx_Q, freq_offset, phase_offset] = rx_offset(tx, fc, Ts, N0, 1, 1);
        ...
        for k = 1 : lent - 1 % combine top PLL th1
            ...
            th2(k + 1) = mu2 * rp(k) * sin(4 * pi * f0 * t(k) + 2 * th1(k) + 2 * th2(k)) - th2(k);
        ...
        % Remove phase offset
        rx_I_shaped__ = rx_I_shaped_ .* cos(th2(end)) - rx_Q_shaped_ .* sin(th2(end));
        rx_Q_shaped__ = rx_Q_shaped_ .* cos(th2(end)) + rx_I_shaped_ .* sin(th2(end));

        % Downsample to get to the sample
        rx_symbols_I = rx_I_shaped__(filter_delay:sampPerSymb:filter_delay+sampPerSymb*N_symbols-1);
        rx_symbols_Q = rx_Q_shaped__(filter_delay:sampPerSymb:filter_delay+sampPerSymb*N_symbols-1);

        bits_I_demod = PAM4_demod(rx_symbols_I);
        bits_Q_demod = PAM4_demod(rx_symbols_Q);
